            ...
            <div class="page-header float-right">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <li><a class="btn btn-primary" href="{{route('post_create')}}"><i class="fa fa-plus"></i></a></li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                ...
                                @foreach($posts as $post)
                                    <tr>
                                        <td>{{$post->id}}</td>
                                        <td>{{ Str::limit(strip_tags($post->title), $limit = 20) }}</td>
                                        ...
                                        <td><span class="btn btn-success btn-sm mr-1"><i class="fa fa-heart-o"></i> {{$post->userLike->count()}}</span>
                                            <span class="btn btn-danger btn-sm mr-1"><i class="fa fa-eye"></i> {{$post->view_count}}</span>
                                        </td>
                                        <td>
